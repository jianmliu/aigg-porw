...
const aggProof = (who) => { const i = aggEntries.findIndex((e) => e.instance === who.toLowerCase()); const l = aggEntries[i].leaf; return { index: i, leaf: { mepId: H(l.mepId), instance: l.instance, partialsRoot: H(l.partialsRoot), coverageBytes: l.coverageBytes, deviceId: H(l.deviceId), signature: H(l.signature) }, proof: V.merkleProof(aggLeaves, i).map(H) }; };
const F = ({
...
`;
  ...
  const leafFn = (name, p) => `    function ${name}() internal pure returns (uint64 index, IPoRWClaimManager.ClaimLeaf memory l, bytes32[] memory proof) { index = ${p.index}; l = IPoRWClaimManager.ClaimLeaf({ mepId: ${p.leaf.mepId}, instance: ${checksum(p.leaf.instance)}, partialsRoot: ${p.leaf.partialsRoot}, coverageBytes: ${p.leaf.coverageBytes}, deviceId: ${p.leaf.deviceId}, signature: hex"${p.leaf.signature.slice(2)}" }); proof = ${name}Proof(); }\n` + arr32(name + "Proof", p.proof);
