...
const run = async (nd, tag) => { let t0 = performance.now(); const st = await nd.loadModel(hdr.name, payload, { steps }); const loadMs = performance.now() - t0;
  t0 = performance.now(); const r = await nd.challenge(st.mep.mepId, ch, { stimulusSeed: 7 }); const slotMs = performance.now() - t0;
...
{ const k = await loadKernelFromBytes(sketchWasm); const mep = { steps: 1000 }; const t0 = performance.now(); const re = Vf.reexecuteLif(k, payload, { stimulusSeed: 7, execDigest: new Uint8Array(32) }, mep); const ms = performance.now() - t0;
