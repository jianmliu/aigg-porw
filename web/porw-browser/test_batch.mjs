...
{ const C = await mk("66"); const rr = await C.nd.batchRunsRoot(C.mep, RUNS); check("a client gets the task's initStateRoot without executing anything (batchRunsRoot)", hex(rr.runsRoot) === hex(rA.result.initStateRoot) && rr.initStateRoots.every((x, k) => hex(x) === hex(rA.runs[k].initStateRoot))); }
