...
//   Claim(bytes32 schemeDigest,bytes32 mepId,bytes32 modelId,bytes32 partialsRoot,uint64 coverageBytes,bytes32 challenge,bytes32 deviceId,bytes32 execDigest,uint32 stimulusSeed)
...
export const TYPES = {
  ...
  Claim: [{ name: "schemeDigest", type: "bytes32" }, { name: "mepId", type: "bytes32" }, { name: "modelId", type: "bytes32" }, { name: "partialsRoot", type: "bytes32" }, { name: "coverageBytes", type: "uint64" }, { name: "challenge", type: "bytes32" }, { name: "deviceId", type: "bytes32" }, { name: "execDigest", type: "bytes32" }, { name: "stimulusSeed", type: "uint32" }],
...
export const claimStructHash = (c) => keccak_256(cat(typeHash("Claim"), c.schemeDigest, c.mepId, c.modelId, c.partialsRoot, wordBig(c.coverageBytes), c.challenge, c.deviceId, c.execDigest, wordBig(c.stimulusSeed)));
...
export const claimMessage = (c) => ({ schemeDigest: hex(c.schemeDigest), mepId: hex(c.mepId), modelId: hex(c.modelId), partialsRoot: hex(c.partialsRoot), coverageBytes: String(c.coverageBytes), challenge: hex(c.challenge), deviceId: hex(c.deviceId), execDigest: hex(c.execDigest), stimulusSeed: String(c.stimulusSeed) });
