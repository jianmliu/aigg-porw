...
const payloadPath = a.payload, steps = Number(a.steps || 2), samples = Number(a.samples || 16), rounds = Number(a.rounds || 3), workers = Number(a.workers || 0);
...
const ISO = { "cross-origin-opener-policy": "same-origin", "cross-origin-embedder-policy": "require-corp" }; // SharedArrayBuffer needs cross-origin isolation
const server = http.createServer((req, res) => {
  const u = new URL(req.url, "http://x");
  if (u.pathname === "/payload.bin") { res.writeHead(200, { ...ISO, "content-type": "application/octet-stream", "content-length": payload.length }); return res.end(Buffer.from(payload.buffer, payload.byteOffset, payload.length)); }
  ...
  res.writeHead(200, { ...ISO, "content-type": mime[path.extname(p)] || "application/octet-stream" }); fs.createReadStream(p).pipe(res);
});
