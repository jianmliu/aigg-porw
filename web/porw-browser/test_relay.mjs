...
const nTiles = Math.floor(payload.length / 4096); const lv = []; for (let t = 0; t < nTiles; t++) lv.push(V.weightsLeaf(t, payload.subarray(t * 4096, (t + 1) * 4096)));
const mep = makeMep({ name: "relay-test", modelId: V.merkleRoot(lv), steps }); const mepHex = V.hex(mep.mepId);
