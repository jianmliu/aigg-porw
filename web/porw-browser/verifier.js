...
export function reexecuteLif(kernel, payloadBytes, run, { stimulusIds = null, silenceIds = null, wUnitQ16 = 0 } = {}) {
  ...
  for (let s = 1; s <= run.steps; s++) { const rc = e.porw_lif_step(bufPtr + hdr.synOffset, hdr.synapses >>> 0, cur, nxt, acc, n >>> 0, s >>> 0, run.stimulusSeed >>> 0, wUnitQ16 >>> 0); if (rc !== 0) throw new Error("lif rc=" + rc); [cur, nxt] = [nxt, cur]; }
