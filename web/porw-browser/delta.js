...
// MAGIC "FLYDELTAv3\0\0" | base model_id | u64 neurons | parent A id | parent B id | u64 seed | u8 granularity | u8 layout | u16 min_syn
// | u32 mut_rate_q32 | u32 mean_ratio_q16 | u16 r_rows | rows | u32 ops | u16 name_len | name | u16 da_len | da | ops
//
// LAYOUT (the byte after granularity). 0 = compact: records below min_syn are dropped and the rest re-packed. 1 = IN PLACE:
// the payload keeps every base record at the base's byte offset, a record the individual lacks has weight 0, and the name has
// the base's byte length — so record j of a child is a function of record j of the base and of its parents' payloads alone
// (`inheritRecord`), which is what lets a wrong declared model_id be shown wrong from one record (proposals/flydelta-inplace).
// In-place lineages are closed: parents are the base (zero id) or in-place crosses with min_syn <= the child's, and nothing
// carries explicit ops. A founder is a cross of the base with itself and mut_rate_q32 = 0xFFFFFFFF ("every record mutates"):
// a fresh draw everywhere, the v2 individual's distribution.
export const LAYOUT = { compact: 0, inplace: 1 }; export const MUT_ALWAYS = 0xffffffff;
...
export function decodeDelta3(bytes) {
  ...
  const granularity = dv.getUint8(124); if (granularity > 2) throw new Error("bad granularity"); const layout = dv.getUint8(125); if (layout > 1) throw new Error("bad layout"); const minSyn = dv.getUint16(126, true); const mutRateQ32 = dv.getUint32(128, true); const meanRatioQ16 = dv.getUint32(132, true); const nrows = dv.getUint16(136, true);
  ...
  const ops = new Array(nops); for (let i = 0; i < nops; i++) ops[i] = readRec(dv, off + i * REC);
  for (let i = 0; i < nops; i++) { if (ops[i].pre >= neurons || ops[i].post >= neurons) throw new Error(`op ${i}: neuron index out of range`); if (i && cmp(ops[i - 1], ops[i]) >= 0) throw new Error(`op ${i}: ops must be sorted by (post, pre) and unique`); }
  if (layout === 1 && nops) throw new Error("an in-place delta carries no explicit ops");
  return { baseModelId, neurons, parentA, parentB, seed, granularity, layout, minSyn, mutRateQ32, meanRatioQ16, rTable, name, baseDA, ops };
}
export function encodeDelta3({ baseModelId, neurons, parentA, parentB, seed, name, baseDA = "", granularity = 0, layout = 0, minSyn = 5, mutRateQ32 = 2 ** 29, meanRatioQ16 = 65536, rTable = DEFAULT_R_TABLE, ops = [] }) {
  if (baseModelId.length !== 32 || parentA.length !== 32 || parentB.length !== 32) throw new Error("ids must be 32 bytes"); if (![0, 1, 2].includes(granularity)) throw new Error("bad granularity"); if (![0, 1].includes(layout)) throw new Error("bad layout"); if (layout === 1 && ops.length) throw new Error("an in-place delta carries no explicit ops");
  ...
  out.set(enc(MAGIC_DELTA3), 0); out.set(baseModelId, 12); dv.setBigUint64(44, BigInt(neurons), true); out.set(parentA, 52); out.set(parentB, 84); dv.setBigUint64(116, BigInt(seed), true); dv.setUint8(124, granularity); dv.setUint8(125, layout);
...
/** The per-record rule of a cross, shared by `genotype` and by anyone re-deriving ONE record (a verifier): pick a parent's
 *  value by one hash bit of the inheritance unit, mutate with probability mut_rate into a fresh draw around the BASE count.
 *  `ctx` = crossContext(d); c = |base weight|; a, b = the parents' values for this record. In-place: a, b are the parents'
 *  payload weights (|w|) and the result is thresholded, so the function maps committed bytes to committed bytes. */
export function crossContext(d) { const lo = Number(d.seed & 0xffffffffn) >>> 0, hi = Number(d.seed >> 32n) >>> 0; const dom = (x) => [(lo ^ x) >>> 0, (hi ^ x) >>> 0]; return { d, pick: dom(DOM_PICK), mut: dom(DOM_MUT), draw: dom(DOM_DRAW), tables: new Map() }; }
export function inheritRecord(ctx, pre, post, c, a, b) {
  const d = ctx.d, FF = 0xffffffff; const [ka, kb] = d.granularity === 0 ? [pre, post] : d.granularity === 1 ? [pre, FF] : [FF, post];
  let v = (hash64Words(ctx.pick[0], ctx.pick[1], ka, kb)[0] >>> 31) === 0 ? a : b;
  if (d.mutRateQ32 === MUT_ALWAYS || hash64Words(ctx.mut[0], ctx.mut[1], pre, post)[0] < d.mutRateQ32) { let t = ctx.tables.get(c); if (!t) { t = nbTable(c, rOf(c, d.rTable), d.meanRatioQ16); ctx.tables.set(c, t); } v = Math.min(32767, sampleFromTable(t, hash64(ctx.draw[0], ctx.draw[1], pre, post))); } // a mutation is a fresh draw around the BASE count: the population stays stationary
  return d.layout === 1 && v < d.minSyn ? 0 : v;
}
/** genotype = the count of every base record of a procedural delta (compact: before min_syn; in-place: the payload's own
 *  |weights|, i.e. already thresholded — picking commutes with thresholding, so both layouts express the same child).
 *  `resolve(idHex)` returns an ancestor's delta bytes. `child`: the delta asking for this one as a parent. */
export function genotype(src, baseMid, deltaBytes, { resolve = null, cache = new Map(), asParent = false, child = null } = {}) {
  const id = V.hex(deltaId(deltaBytes)); const v3 = isDelta3(deltaBytes); if (!v3 && !isDelta2(deltaBytes)) throw new Error("a genotype needs a procedural delta (v2 or v3)"); const d = v3 ? decodeDelta3(deltaBytes) : decodeDelta2(deltaBytes);
  ...
  if (child) { const pl = v3 ? d.layout : 0; if (pl !== child.layout) throw new Error("a lineage keeps one layout: " + (child.layout ? "an in-place child needs in-place parents" : "a compact child needs compact parents")); if (child.layout === 1 && d.minSyn > child.minSyn) throw new Error(`an in-place child's min_syn (${child.minSyn}) must be >= its parents' (${d.minSyn})`); }
  if (cache.has(id)) return cache.get(id);
  let g;
  if (!v3) g = sampleCounts(src, d.seed, d.meanRatioQ16, d.rTable);
  else {
    const par = (pid) => { if (isZero(pid)) { const c = new Int32Array(src.length); for (let i = 0; i < src.length; i++) c[i] = Math.abs(src[i].w); return c; } const b = resolve && resolve(V.hex(pid)); if (!b) throw new Error(`parent delta ${V.hex(pid)} not provided`); return genotype(src, baseMid, b, { resolve, cache, asParent: true, child: d }); };
    const gA = par(d.parentA), gB = par(d.parentB); const ctx = crossContext(d); g = new Int32Array(src.length);
    for (let i = 0; i < src.length; i++) g[i] = inheritRecord(ctx, src[i].pre, src[i].post, Math.abs(src[i].w), gA[i], gB[i]);
  }
...
export function applyProcedural(base, deltaBytes, { baseModelId = null, resolve = null, cache = new Map() } = {}) {
  ...
  if (d.layout === 1) { // in place: the base's bytes with this name and these weights; nothing moves
    const nameB = enc(d.name); if (nameB.length !== h.synOffset - 30 - h.neurons * 8) throw new Error(`an in-place name must have the base name's byte length (${h.synOffset - 30 - h.neurons * 8}), got ${nameB.length}: use fitName`);
    const out = Uint8Array.from(base); out.set(nameB, 30); const dv = new DataView(out.buffer);
    for (let i = 0; i < src.length; i++) dv.setInt16(h.synOffset + i * REC + 8, src[i].w < 0 ? -g[i] : g[i], true); // g is already thresholded
    return out;
  }
...
/** a name of exactly `len` bytes for an in-place delta: truncated, or padded with "_" */
export function fitName(name, len) { let b = enc(name); if (b.length > len) { b = b.slice(0, len); while (b.length && (b[b.length - 1] & 0xc0) === 0x80) b = b.slice(0, -1); } return dec(b) + "_".repeat(len - b.length); }
/** the byte length an in-place name must have for this base */
export const baseNameLength = (base) => { const h = decodeHeader(base); return h.synOffset - 30 - h.neurons * 8; };
/** record j of a payload, read from its bytes: { pre, post, w } */
export function recordAt(payload, j) { const h = decodeHeader(payload); const dv = new DataView(payload.buffer, payload.byteOffset, payload.byteLength); return readRec(dv, h.synOffset + j * REC); }
/** what record j of an in-place child must be, from record j of the base and of the two parents' payloads (null = the base) */
export function expectedRecord(deltaBytes, base, parentAPayload, parentBPayload, j, ctx = null) {
  const d = decodeDelta3(deltaBytes); if (d.layout !== 1) throw new Error("expectedRecord is defined for the in-place layout"); const r = recordAt(base, j); const c = Math.abs(r.w);
  const val = (p) => (p ? Math.abs(recordAt(p, j).w) : c); const v = inheritRecord(ctx || crossContext(d), r.pre, r.post, c, val(parentAPayload), val(parentBPayload)); return { pre: r.pre, post: r.post, w: r.w < 0 ? -v : v };
}
